...
describe("Multi-selection — Phase 6.5.1 store contract", () => {
  it("starts empty (fresh state)", () => {
    ...
    expect(s.multiSelection).toEqual({ nodeIds: [], pipeIds: [], dimensionIds: [] });
  });
  ...
  it("select(single) sets both legacy and multiSelection to that target", () => {
    ...
    expect(s.multiSelection).toEqual({ nodeIds: ["n1"], pipeIds: [], dimensionIds: [] });
    ...
    expect(s2.multiSelection).toEqual({ nodeIds: [], pipeIds: ["p1"], dimensionIds: [] });
  });
