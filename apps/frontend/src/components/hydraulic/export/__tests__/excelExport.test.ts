...
describe("Phase 13.26 — hydraulic sheet matches УДДТ-5 reference layout", () => {
  it("emits the УДДТ-5 column layout (A-S = 19 cols)", () => {
    const wb = exportAndRead(makeStateWithFixture());
    const ws = wb.Sheets["Халаалт гидравлик"]!;
    const rows = XLSX.utils.sheet_to_json<unknown[]>(ws, { header: 1 });
    const row1 = rows[0] as (string | null)[];
    const row2 = rows[1] as (string | null)[];
    // Row 1 — group headers in template positions A-D
    ...
    // Row 2 — column headers in template positions D-S
    expect(row2[3]).toBe("Q [кВт]");
    expect(row2[4]).toBe("G [кг/с]");
    expect(row2[5]).toBe("G [т/цаг]");
    expect(row2[6]).toBe("L [м]");
    expect(row2[7]).toBe("1+α");
    expect(row2[8]).toBe("di [мм]");
    expect(row2[9]).toBe("di [м]");
    expect(row2[10]).toBe("dст [мм]");
    expect(row2[11]).toBe("ξ");
    expect(row2[12]).toBe("W [м/с]");
    expect(row2[13]).toBe("Rш [Pa/м]");
    expect(row2[14]).toBe("Lэ [м]");
    expect(row2[15]).toBe("∆Pi [Pa]");
    expect(row2[16]).toBe("∆Hi [м]");
    expect(row2[17]).toBe("Lэкв");
    expect(row2[18]).toBe("Шугамын төрөл");
  });
  ...
  it("Phase 13.26 — Q (kW) populated from to-node heatLoad when no solver results", () => {
    // Engineer report fix: when the consumer node carries heatLoad_w
    // but no solve has run yet, the pipe row whose to-node is the
    // consumer should reflect that load. Fixture pipe k1→c1 has
    // c1.heatLoad_w = 405_000 → row Q = 405 kW.
    const wb = exportAndRead(makeStateWithFixture());
    const ws = wb.Sheets["Халаалт гидравлик"]!;
    const rows = XLSX.utils.sheet_to_json<unknown[]>(ws, { header: 1 });
    // Heating sheet rows: row 0/1 = headers; row 2 = p0 (s1→k1);
    // row 3 = p1 (k1→c1, the consumer pipe). c1 has 405 kW load.
    const consumerPipeRow = rows[3] as unknown[];
    expect(typeof consumerPipeRow[3]).toBe("number"); // Q [kW] col D
    expect(consumerPipeRow[3] as number).toBe(405);
  });

  it("Phase 13.25 — Шугамын төрөл column at position S contains 'Магистраль' / 'Салбар'", () => {
    const wb = exportAndRead(makeStateWithFixture());
    const ws = wb.Sheets["Халаалт гидравлик"]!;
    const rows = XLSX.utils.sheet_to_json<unknown[]>(ws, { header: 1 });
    const firstData = rows[2] as unknown[];
    expect(firstData[18]).toMatch(/Магистраль|Салбар|Холбоогүй/);
  });
});
