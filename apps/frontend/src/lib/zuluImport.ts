...
// Phase 7.1 — CP1251 decoder lifted to a shared `encoding` module so
// the DXF import path can reuse the same mapping. The sql.js flow
// still receives JS byte-strings (each char-code = one CP1251 byte),
// so we use the `*FromByteString` variant.
import { decodeCP1251FromByteString } from "./encoding";
...
function decodeIfString(v: SqlValue | undefined): SqlValue {
  ...
  return typeof v === "string" ? decodeCP1251FromByteString(v) : v;
}
